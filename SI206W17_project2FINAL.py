...
try:
	...
	cache_contents = CACHE_FNAME.read()
...
def get_umsi_data():
	if "umsi_directory_data" in CACHE_DICTION:
		...
		for loop in range(12):
			new_page_url = base_url + str(loop)
			html_text = requests.get(new_page_url, headers = {'User-Agent': 'SI_CLASS'}).text
			HTML_lst.append(html_text)
